...
export type ExecuteTempOptions = {
  sql: string;
  userConnId: string;
  pagination?: PaginationInput;
  allowWrite?: boolean;
...
export type PreviewResult = { previewText: string; previewInline: string };
...
export type ExplainTempOptions = {
  sql: string;
  userConnId: string;
  format: 'text' | 'json';
  analyze?: boolean;
...
export type EnumOptionsResult = { options: string[]; count: number };
...
const ensureVarsDefined = (sql: string, vars: SavedQueryVariableDef[]) => {
  try {
    ...
    if (missing.length > 0)
      throw new QueryError('变量缺失', { code: 'vars_missing', missing });
  } catch (e) {
...
function recordRecent(record?: RecentRecord) {
  ...
  const title =
    record.title?.trim() ||
    (record.source === 'saved-sql' ? 'Saved SQL' : 'Ad-hoc query');
    ...
    async (db) => {
      ...
      if (pagination.enabled) {
        ...
        if (Number.isFinite(totalRowsValue)) {
          ...
          execResult.totalPages = Math.max(
            1,
            Math.ceil((totalRowsValue ?? 0) / pageSize)
          );
...
const loadSaved = async (id: string): Promise<SavedSqlRecord> => {
  ...
  if (!saved)
    throw new QueryError('未找到指定的 Saved SQL', { code: 'not_found' });
  return saved;
};
...
export async function executeSavedSql(
  opts: ExecuteOptions
...
export async function executeTempSql(
  opts: ExecuteTempOptions
): Promise<ExecuteResult> {
...
export async function explainSavedSql(
  opts: ExplainOptions
...
export async function explainTempSql(
  opts: ExplainTempOptions
): Promise<ExplainResult> {
  const normalized = (opts.sql ?? '').trim();
  const cleaned = stripTrailingSemicolons(normalized);
  if (!cleaned) {
  ...
  const rows = await withReadonlySession<Array<Record<string, unknown>>>(
    ...
    async (db) => {
      const rows = await db.select(
        stripTrailingSemicolons(compiled.text),
        compiled.values
      );
      return Array.isArray(rows)
        ? (rows as Array<Record<string, unknown>>)
...
const shiftParamPlaceholders = (sql: string, offset: number) =>
  offset === 0
    ? sql
    : sql.replace(/\$(\d+)/g, (_m, g1) => '$' + (Number(g1) + offset));

export async function computeCalcSql(opts: CalcOptions): Promise<CalcResult> {
  const saved = await loadSaved(opts.savedId);
  if (!isReadOnlySelect(saved.sql)) {
    throw new QueryError('基础 SQL 必须为只读', {
      code: 'base_sql_must_be_readonly',
    });
  }
  if (!isReadOnlySelect(opts.calcSql)) {
    throw new QueryError('计算 SQL 必须为只读', {
      code: 'calc_sql_must_be_readonly',
    });
  }
  ensureVarsDefined(saved.sql, saved.variables);
  const baseCompiled = compileSql(saved.sql, saved.variables, opts.values);
  const calcSqlPrepared = opts.calcSql.replace(
    /\{\{\s*_sql\s*\}\}/g,
    'select * from rdv_base'
  );
  ensureVarsDefined(calcSqlPrepared, saved.variables);
  const calcCompiled = compileSql(
    calcSqlPrepared,
    saved.variables,
    opts.values
  );
  ...
  const finalSqlRaw = `with rdv_base as ( ${shiftParamPlaceholders(
    baseForEmbed,
    calcCompiled.values.length
  )} ) ${calcCompiled.text}`;
