...
export default function BrowsePage() {
  ...
  useEffect(() => {
    ...
    readSchemaCache(userConnId)
      .then((res) => {
        ...
        try {
          target = JSON.parse(
            localStorage.getItem('rdv.lastBrowseTarget') || '{}'
          );
        } catch {}
        const hasTarget =
          target.schema &&
          target.table &&
          tbs.some(
            (t) => t.schema === target.schema && t.name === target.table
          );
        if (hasTarget) {
  ...
  const currentTableMeta = useMemo(
    () => tables.find((t) => t.schema === schema && t.name === table) || null,
    [tables, schema, table]
  );
  useEffect(() => {
  ...
  function classify(dt: string): 'json' | 'text' | 'number' | 'date' {
    ...
    if (/(int|numeric|decimal|real|double|money|bigint|smallint)/.test(s))
      return 'number';
      ...
      if (kind === 'json') {
        if (v.startsWith('@>')) {
          ...
          wh.push({
            kind: 'json_contains',
            left,
            right: { kind: 'param', value: jsonText },
          });
          continue;
        }
  ...
  const run = async () => {
    ...
    try {
      ...
      const alias = 't';
      const from = {
        schema: currentTableMeta.schema,
        name: currentTableMeta.name,
        alias,
      };
      const columns: ColumnSelect[] = currentTableMeta.columns.map((c) => ({
        kind: 'column',
        ref: { kind: 'colref', table: alias, name: c.name },
      }));
      const where: WhereOp[] = parseFilters(currentTableMeta, alias, filters);
      let orderBy: OrderByItem[] | undefined = undefined;
      if (Array.isArray(sorting) && sorting.length > 0) {
        orderBy = sorting.map((s: any) => ({
          expr: { kind: 'colref', table: alias, name: String(s.id) },
          dir: s.desc ? 'DESC' : 'ASC',
        }));
      } else {
        ...
        if (pks.length > 0)
          orderBy = pks.map((c) => ({
            expr: { kind: 'colref', table: alias, name: c.name },
            dir: 'ASC' as const,
          }));
      }
      const ast: SelectAst = {
  ...
  useEffect(() => {
    if (currentTableMeta) run();
  }, [currentTableMeta, page, pageSize]);
  useEffect(() => {
    if (currentTableMeta) run();
  }, [sorting]);
    ...
    <Stack gap="md">
      <Group justify="space-between">
        <div>
          ...
          <Text c="dimmed">
            M3 MVP：排序/筛选/分页与 SQL 预览；Keyset 跳页后续增强。
          </Text>
        </div>
        <Group>
          <ActionIcon
          ...
          jsonColumns={
            currentTableMeta
              ? currentTableMeta.columns
                  .filter((c) => /json/i.test(c.dataType))
                  .map((c) => c.name)
              : []
          }
          height={480}
      ...
      <Paper withBorder p="sm">
        ...
        {paramsPreview.length > 0 && (
          <div style={{ marginTop: 6 }}>
            <Text size="sm" c="dimmed">
              参数：{JSON.stringify(paramsPreview)}
            </Text>
          </div>
        )}
      </Paper>
