...
} from '@/lib/assistant/conversation-utils';
...
function useSchemaMetadata(): SchemaMetadataSnapshot | null {
  const [snapshot, setSnapshot] = useState<SchemaMetadataSnapshot | null>(() =>
    getSchemaMetadataSnapshot()
  );
  useEffect(() => {
...
function toUiMessages(
  messages: AssistantConversationMessage[] | undefined
): UIMessage[] {
...
export default function AssistantPage() {
  ...
  const [profileSelection, setProfileSelection] =
    useState<AssistantProfileSelection | null>(null);
  ...
  const initialMessages = useMemo(
    () => toUiMessages(activeConversation?.messages),
  ...
  const handleToggleContext = useCallback(
    (chunk: AssistantContextChunk, checked: boolean) => {
      setSelectedIds((prev) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const loadedSelection = await loadAssistantProfileSelection(
          loadedProfiles
        );
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const keyPresent = await hasAssistantApiKey(
          provider,
          runtime.profile.id
        );
  ...
  const handleProfilesSaved = useCallback(
    (
      nextProfiles: AssistantProviderProfile[],
      nextSelection?: AssistantProfileSelection
    ) => {
      setProfiles(nextProfiles);
      const resolution = resolveAssistantRuntimeSettings(
        nextProfiles,
        nextSelection ?? profileSelection
      );
  ...
  const contextSummaryMap = useMemo(() => {
    ...
    for (const message of messages) {
      if (
        typeof message.contextSummary === 'string' &&
        message.contextSummary.trim().length > 0
      ) {
  ...
  const modelOptions = useMemo(
    () =>
      runtime.profile.models.map((model) => ({
        value: model.id,
