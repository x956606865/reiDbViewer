...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { databases, schemas, tables, ddls, indexes } = await introspectPostgres(dsn)

    // Write to schema cache (best-effort)
    const saved = await saveSchemaCache(userId, userConnId, { databases, schemas, tables, ddls, indexes })
    ...
    return NextResponse.json({ databases, schemas, tables, ddls, indexes, cacheStatus, ...extra })
  } catch (e: any) {
...
async function introspectPostgres(connectionString: string): Promise<{
  ...
  try {
    ...
    const tableKeySet = new Set(tableCols.keys())
    ...
    const ixResA = (await client.query(
    ...
    const ixResB = (await client.query(
      `SELECT
    ...
    for (const r of (ixResB.rows as IxRowB[])) {
      const tkey = key(r.schema, r.table)
    ...
    for (const r of (ixResA.rows as IxRowA[])) {
      const tkey = key(r.schema, r.table)
      if (!tableKeySet.has(tkey)) continue
      const indexes = ensureIndexMap(tkey)
